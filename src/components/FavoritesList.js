...
export default function FavoritesList(props) {
	...
	return <ul className="favorites-list">
		{props.favoritePlaces.map(p => <li key={p.id} className="saved-place" data-lat={p.lat} data-lng={p.lng} onClick={() => props.onNavitateToPlace({ address: p.name, lat: p.lat, lng: p.lng})}>
